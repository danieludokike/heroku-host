...
def registration_page_view(request):
    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            form = RegistrationForm()
            template = 'webapp/account/register.html'
...
def login_page_view(request):
    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if user is not None:
                login(request, user)
                return redirect('web_class:user_profile')
            else:
            ...
            template = 'webapp/account/login.html'
...
def logout_view(request):
    """Logs the User out"""
    logout(request)
    messages.add_message(request, messages.INFO, 'Hello {}, thanks for taking courses in our platform...!! Hope it '
                                                 'was interesting?'.format('Learned Programmer'))
    return redirect('web_class:login_required')
...
def user_profile_page_view(request):
    ...
    query_set = TutorialCourses.objects.all()[:4]
    template = 'webapp/account/profile.html'
...
def user_icon_profile_view(request):
    """Shows the User's icon and details"""
    template = 'webapp/account/user_profile.html',
    context = {
...
def login_required_redirection_view(request):
    ...
    if request.user.is_authenticated:
        ...
        template = 'webapp/account/login_redirect.html'
...
def tutorial_page_view(request, course_name):
    ...
    template = 'webapp/account/tutorial_display.html'
